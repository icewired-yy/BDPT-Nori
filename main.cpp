...
static void renderBlock(const Scene *scene, Sampler *sampler, ImageBlock &block, ImageBlock &lightImage) {
    ...
    for (int y=0; y<size.y(); ++y) {
        for (int x=0; x<size.x(); ++x) {
            for (uint32_t i=0; i<sampler->getSampleCount(); ++i) {
                ...
                /*
                /* Create an empty vector for storing the other contribution */
                std::vector<Color3f> otherContribution;

                /* Compute the incident radiance */
                Color3f contribution = value * integrator->Li(scene, sampler, ray, otherContribution);

                /* Store in the image block */
                block.put(pixelSample, contribution);

                /* Collect the rest contribution */
                if (!otherContribution.empty())
                  for (Color3f& contri : otherContribution)
                      block.put(pixelSample, value * contri);
            }
        }
    }
    /* Add the light image */
    block.putLightImage(lightImage);
    // block.topLeftCorner(size.y(), size.x()) += lightImage.block(offset.y(), offset.x(), size.y(), size.x());
}
...
static void render(Scene *scene, const std::string &filename) {
    ...
    scene->getIntegrator()->preprocess(scene, lightImage);
    lightImage.asLightImage();
    ...
    std::thread render_thread([&] {
        ...
        auto map = [&](const tbb::blocked_range<int> &range) {
            ...
            for (int i=range.begin(); i<range.end(); ++i) {
                ...
                renderBlock(scene, sampler.get(), block, lightImage);
