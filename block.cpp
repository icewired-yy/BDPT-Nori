...
void ImageBlock::asLightImage()
{
    auto size_y = getSize().y();
    auto size_x = getSize().x();
    for (int y = 0; y < size_y; y++)
        for (int x = 0; x < size_x; x++)
            coeffRef(y, x).w() = 0.0f;
            /*if (coeffRef(y, x).x() == 0 && coeffRef(y, x).y() && coeffRef(y, x).z())
                coeffRef(y, x).w() = 0.0f;
            else coeffRef(y, x).w() = 1.0f;*/
            
}

void ImageBlock::putLightImage(ImageBlock &light)
{
    Vector2i offset = m_offset - light.getOffset() +Vector2i::Constant(light.getBorderSize() - m_borderSize);
    Vector2i size = getSize() +Vector2i(2 * getBorderSize());

    for (int y = 0; y < size.y(); y++)
        for (int x = 0; x < size.x(); x++)
            coeffRef(y, x) += light.coeffRef(y + offset.y(), x + offset.x()) * coeffRef(y, x).w();
}
